#!/usr/bin/env python3
"""
Reference implementation of standard operations using pandas
All other engines should produce identical results
"""
import pandas as pd
import json
...
class ReferenceImplementation:
    """Reference implementation that defines the expected behavior"""
    ...
    def write_results(self):
        ...
        output_dir = Path(__file__).parent / 'reference_output'
...
def get_expected_results(data_paths):
    """Get expected results from reference implementation"""
    ref = ReferenceImplementation(data_paths)
    
    results = {}
    results['load_data'] = ref.load_data()
    results['filter_and_aggregate'] = ref.filter_and_aggregate()
    results['join_datasets'] = ref.join_datasets()
    results['complex_analytics'] = ref.complex_analytics()
    results['write_results'] = ref.write_results()
